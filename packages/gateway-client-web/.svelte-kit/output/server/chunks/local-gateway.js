...
const ADAPTIVE_AGENT_HOME = join(homedir(), ".adaptiveAgent");
join(ADAPTIVE_AGENT_HOME, "data", "gateway");
const GATEWAY_CONFIG_PATH = join(ADAPTIVE_AGENT_HOME, "config", "gateway.json");
const AGENT_CONFIG_DIR = join(ADAPTIVE_AGENT_HOME, "agents");
join(AGENT_CONFIG_DIR, "default-agent.json");
const DEFAULT_GATEWAY_JWT_SECRET = "adaptive-agent-local-dev-secret";
async function loadLocalGatewayJwtAuthConfig() {
...
async function loadLocalGatewayConnectionConfig() {
  const rawConfig = await loadLocalGatewayConfig();
  ...
  return {
    ...
    port: typeof server.port === "number" && Number.isInteger(server.port) && server.port > 0 ? server.port : void 0,
    websocketPath: readOptionalString(server.websocketPath)
  };
}
async function loadLocalGatewayConfig() {
  if (!existsSync(GATEWAY_CONFIG_PATH)) {
    return void 0;
  }
  return JSON.parse(await readFile(GATEWAY_CONFIG_PATH, "utf-8"));
}
function readAudience(value) {
  ...
  const audience = value.filter((entry) => typeof entry === "string" && entry.trim().length > 0);
  return audience.length > 0 ? audience : void 0;
}
...
async function resolveLocalDevJwtConfig(options = {}) {
  const gatewayAuth = await loadLocalGatewayJwtAuthConfig();
  const secretSource = options.secret ? "option" : process.env.GATEWAY_JWT_SECRET ? "env" : gatewayAuth?.secret ? "gateway-config" : "default";
  return {
...
async function mintLocalDevJwt(options) {
  ...
  let jwt = new SignJWT(payload).setProtectedHeader({ alg: "HS256", typ: "JWT" }).setSubject(options.subject).setIssuedAt().setExpirationTime(options.expiresIn ?? "7d");
  if (config.issuer) {
...
async function loadLocalGatewayDefaults() {
  const connection = await loadLocalGatewayConnectionConfig().catch(() => void 0);
...
async function mintGatewayBrowserToken(input) {
...
export {
  loadLocalGatewayDefaults as l,
  mintGatewayBrowserToken as m
};
